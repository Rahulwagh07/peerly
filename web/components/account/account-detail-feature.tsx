...
const AccountDetailFeature: React.FC = () => {
  ...
  const programId = useMemo(
    () => getLendingProgramId(cluster.network as Cluster),
    [cluster],
  );
  ...
  const loadAccountDetails = async () => {
    ...
    try {
      ...
      const accountType = accountDetails.accountType.borrower
        ? 'Borrower'
        : accountDetails.accountType.lender
          ? 'Lender'
          : 'None';
      ...
      if (accountType === 'Borrower') {
        ...
        processedLoans = allUserAccounts
          ...
          .flatMap((userAccount) =>
            userAccount.account.loans.filter(
              (loan: any) => loan.lender.toBase58() === address.toBase58(),
            ),
          );
      }
    ...
    <div className="mx-2 sm:mx-auto  w-11/12 relative overflow-hidden py-16  -mt-24 sm:-mt-24 lg:py-12">
      ...
      <div>
        <CardHeader className="flex items-center justify-center  mt-2 sm:mt-12">
          ...
          <CardDescription>
            See all your loan details that you are part of.
          </CardDescription>
        </CardHeader>

        <div className="flex items-center justify-center mb-2">
          <Card
            className="hidden sm:flex gap-4 p-3 px-16 items-center justify-center
        ...
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead
                  className={`${accountType === 'Borrower' ? 'hidden' : ''}`}
                >
                  Borrower
                </TableHead>
                <TableHead
                  className={`${accountType === 'Lender' ? 'hidden' : ''}`}
                >
                  Lender (Funded By)
                </TableHead>
            ...
            <TableBody>
              {loans.map((loan, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell
                    onClick={
                      formatStatus(loan.status) !== 'Requested'
                        ? () => handleCopy(loan.lender.toString())
                        : undefined
                    }
                    className={`${
                      formatStatus(loan.status) !== 'Requested'
                  ...
                  <TableCell>
                    {loan.requestDate
                      ? formatDateFromBN(loan.requestDate)
                      : '---'}
                  </TableCell>
                  <TableCell>
                    {loan.dueDate ? formatDateFromBN(loan.dueDate) : '---'}
                  </TableCell>
                  <TableCell>
                    {loan.repayDate ? formatDateFromBN(loan.repayDate) : '---'}
                  </TableCell>
